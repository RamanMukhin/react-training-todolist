...
const TodoItem = ({ id, title, done = false, chosen = false, deleteTodoItem, updateTodoItem }) => {
    ...
    return (
        <div className='TodoItem'>
            <div className='Square' onClick={updateChosenProp}>{chosen ? 'x' : ''}</div>
            {
                done ? <p className='CrossedText'>{title}</p> : <p>{title}</p>
            }
            <div className='Square' onClick={updateDoneProp}>{done ? 'v' : ''}</div>
            <div className='Square' onClick={updateTitleProp}><p>fix</p></div>
            <div className='Square' onClick={deleteCurrentTodoItem}><p>del</p></div>
        </div>
    );
